...
namespace Oriflame.PolicyBuilder.Xml.Definitions.Sections
{
    ...
    {
        public RetryPolicy(string condition, int count, TimeSpan interval, bool firstFastRetry = false)
            : this(condition, count.ToString(), interval.GetSeconds(), firstFastRetry)
        {
        }

        public RetryPolicy(string condition, string count, TimeSpan interval, bool firstFastRetry = false)
            : this(condition, count, interval.GetSeconds(), firstFastRetry)
        {
        }

        public RetryPolicy(string condition, int count, string interval, bool firstFastRetry = false)
        ...
        public RetryPolicy(string condition, string count, string interval, bool firstFastRetry = false)
            : base("retry")
        {
